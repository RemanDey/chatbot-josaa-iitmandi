...
STOP_WORDS = {
    "what", "is", "the", "a", "an", "at", "in", "of", "for", "to", "how",
    "do", "i", "me", "my", "please", "can", "tell", "about", "are", "does",
    "which", "where", "when", "who", "why", "its", "it", "this", "that",
    "and", "or", "but", "on", "with", "as", "by", "from", "be", "was", "will"
}
...
class ConfidenceScorer:
    ...
    def score_chunks(self, query: str, chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Score each chunk and filter out those below the threshold (0.38).
        ...
        for chunk in chunks:
            ...
            # 1. Freshness score (weight = 0.1)
            # Parse ingested_at ISO timestamp.
            # E.g. '2026-05-22T18:05:06.123456+00:00'
            freshness = 0.7  # Default if missing or invalid
            ...
            # Apply specific weight bonuses for preferred files
            if source.endswith(".md"):
                authority = min(1.0, authority + 0.2)
            if "Curriculum Explanation Session (B.TechB.S. 2025) Updated.pdf" in source:
                authority = min(1.0, authority + 0.3)
            if "CnP Report till 29-07-2024.pdf" in source:
                authority = 1.0  # Max authority for this official placement report
            
            # Additional priority for JOSAA and Branch guides
            if "jossa_sounseeling_guide" in source.lower() or "josaa" in source.lower():
                authority = min(1.0, authority + 0.3)
            if "branch comaprisan" in source.lower() or "branch comparison guide" in source.lower():
                authority = min(1.0, authority + 0.3)
            ...
            # Filter: Keep only chunks with confidence_score >= 0.38
